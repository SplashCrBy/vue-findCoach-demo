...
export default {
  async contactCoach(context, payload) {
    ...
    const response = await fetch(
      `https://coach-50a3d-default-rtdb.firebaseio.com/requests/${payload.coachId}.json`,
      {
        method: 'POST',
  ...
  async fetchRequests(context) {
    ...
    const response = await fetch(
      `https://coach-50a3d-default-rtdb.firebaseio.com/requests/${coachId}.json?Auth` + token
    );
    const responseData = await response.json();
